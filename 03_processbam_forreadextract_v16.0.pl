...
#	-16.0 = 16 April 2020
# 				Modify the script to work for cram files		
#		
\n";
...
sub extractreads_bam {
	...
	foreach my $indivi (@indifiles) {
		...
		unless (-e "$path/Allreadsforbam/$file/$allreadindi.allreadIDs.bam") {
			#Need to add reference sequence for cram files
			system ("java -jar $picardtools/picard.jar FilterSamReads INPUT=$bamlocation/$bam_id VALIDATION_STRINGENCY=LENIENT FILTER=includeReadList REFERENCE_SEQUENCE=$GENOME READ_LIST_FILE=$path/Allreadsforbam/$file/$allreadindi.allreadIDs.txt WRITE_READS_FILES=false OUTPUT=$path/Allreadsforbam/$file/$allreadindi.allreadIDs.bam") == 0 or die ("unable to run picard tools in $file on $allreadindi \n");
